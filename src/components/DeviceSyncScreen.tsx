    ...
    <div id="device-sync-screen" className="flex flex-col pb-24 px-3 pt-2 max-w-md mx-auto space-y-4">
      ...
      <div className={`rounded-3xl p-4 shadow-xl border transition-colors ${isLight ? 'bg-white/95 border-sky-200 shadow-sky-100/70' : 'bg-slate-900 border-emerald-900/50'}`}>
        ...
        <div className="grid grid-cols-2 gap-2.5 my-3">
          <div className={`p-3 rounded-2xl border ${isLight ? 'bg-sky-50/70 border-sky-200' : 'bg-slate-950/80 border-slate-800'}`}>
            <div className="flex justify-between text-[10px] font-semibold mb-1">
              <span className="flex items-center gap-1"><Signal className="w-3.5 h-3.5 text-sky-500" /> RSSI</span>
              <span className={signalQuality.color}>{signalQuality.text}</span>
            </div>
            <div className="flex items-baseline gap-1">
              <span className="text-lg font-black font-mono">{espState.rssi}</span>
              <span className="text-[10px] opacity-60">dBm</span>
            </div>
            <div className={`w-full h-1.5 rounded-full mt-2 ${isLight ? 'bg-sky-200' : 'bg-slate-800'}`}>
              <div className="bg-emerald-500 h-full rounded-full transition-all" style={{ width: `${signalQuality.pct}%` }} />
            </div>
          </div>

          <div className={`p-3 rounded-2xl border ${isLight ? 'bg-sky-50/70 border-sky-200' : 'bg-slate-950/80 border-slate-800'}`}>
            <div className="text-[10px] font-semibold mb-1 flex items-center gap-1">
              {espState.connectionType === 'wifi' ? <Wifi className="w-3.5 h-3.5 text-emerald-500" /> : <Bluetooth className="w-3.5 h-3.5 text-sky-500" />}
              MODO {espState.connectionType.toUpperCase()}
            </div>
            {isEditingIp ? (
              <div className="flex gap-1">
                <input type="text" value={customIp} onChange={(e) => setCustomIp(e.target.value)} className="w-full text-[10px] px-1 py-0.5 rounded border focus:outline-none bg-slate-900 text-white" />
                <button onClick={() => { esp32.setIpAddress(customIp); setIsEditingIp(false); }} className="px-1.5 bg-emerald-600 text-white rounded font-bold">OK</button>
              </div>
            ) : (
              <div onClick={() => setIsEditingIp(true)} className="text-[11px] font-mono font-bold text-emerald-500 cursor-pointer">{espState.ipAddress || 'SEM IP'}</div>
            )}
            <div className="text-[9px] font-mono opacity-50 mt-1 truncate">{espState.wifiMac || espState.bleMac || 'MAC: --'}</div>
          </div>
        </div>
      </div>
      ...
      <div className={`rounded-3xl p-4 shadow-xl border transition-colors ${isLight ? 'bg-white/95 border-sky-200 shadow-sky-100/70' : 'bg-slate-900 border-slate-800'}`}>
        ...
        <div className="space-y-3">
          ...
          <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Senha Wi-Fi" className="w-full p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white" />
