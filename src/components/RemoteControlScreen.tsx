      ...
      >
        ...
        {activeLayout === 'ac' && (
          <div id="ac-remote-layout" className="w-full flex flex-col items-center space-y-3.5 animate-in fade-in duration-200">
            {/* AC LCD DISPLAY PANEL */}
            <div
              id="ac-lcd-display"
              className={`w-full rounded-2xl p-3.5 border font-mono relative overflow-hidden transition-all shadow-inner ${
                isLight
                  ? 'bg-gradient-to-b from-sky-50 via-cyan-50/70 to-emerald-50/50 border-cyan-300 text-slate-800 shadow-sky-200/50'
                  : 'bg-gradient-to-b from-slate-950 via-emerald-950/20 to-slate-950 border-emerald-500/30 text-emerald-300 shadow-black'
              }`}
            >
              ...
              {/* Header inside LCD: Power & Clock & Timers */}
              <div className="flex items-center justify-between text-[11px] font-bold pb-2 border-b border-dashed border-cyan-400/30 relative z-10">
                <div className="flex items-center gap-1.5">
                  ...
                  />
                  <span className={acPower ? (isLight ? 'text-emerald-700 font-extrabold' : 'text-emerald-400') : 'text-slate-400'}>
                    {acPower ? 'ON' : 'STANDBY'}
                  </span>
                </div>

                {/* Clock & Timer Indicators */}
                <div className="flex items-center gap-2 text-[10px]">
                  {acTimerOn && (
                    <span className="px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-600 font-bold border border-emerald-500/30">
                      T-ON {acTimerMinutes / 60}h
                    </span>
                  )}
                  {acTimerOff && (
                    <span className="px-1.5 py-0.5 rounded bg-rose-500/20 text-rose-600 font-bold border border-rose-500/30">
                      T-OFF {acTimerMinutes / 60}h
                    </span>
                  )}
                  <span className={`font-mono ${isLight ? 'text-slate-600 font-semibold' : 'text-slate-400'}`}>
                    🕒 {acClockTime}
                  </span>
                </div>
              </div>

              {/* Center LCD: Temperature & Mode Icons */}
              <div className="flex items-center justify-between py-2 relative z-10">
                <div className="flex items-baseline">
                  <span className={`text-4xl font-extrabold tracking-tight ${
                    isLight ? 'text-cyan-900' : 'text-emerald-300'
                  }`}>
                    {acPower ? acTemp : '--'}
                  </span>
                  <span className={`text-xl font-bold ml-1 ${isLight ? 'text-cyan-700' : 'text-emerald-400'}`}>
                    °C
                  </span>
                </div>
                  ...
                  <div className="flex items-center gap-1 text-xs font-bold uppercase tracking-wide">
                    {acMode === 'cool' && (
                      <span className="flex items-center gap-1 text-cyan-600 dark:text-cyan-400">
                        <Snowflake className="w-4 h-4 animate-spin-slow" /> COOL ({strings.remote.acModeCool})
                      </span>
                    )}
                    {acMode === 'heat' && (
                      <span className="flex items-center gap-1 text-orange-600 dark:text-orange-400">
                        <Flame className="w-4 h-4" /> HEAT ({strings.remote.acModeHeat})
                      </span>
                    )}
                    {acMode === 'auto' && (
                    ...
                    {acMode === 'dry' && (
                      <span className="flex items-center gap-1 text-sky-600 dark:text-sky-400">
                        <Droplets className="w-4 h-4" /> DRY ({strings.remote.acModeDry})
                      </span>
                    )}
                    {acMode === 'fan' && (
                      <span className="flex items-center gap-1 text-indigo-600 dark:text-indigo-400">
                        <Fan className="w-4 h-4 animate-spin" /> FAN ({strings.remote.acModeFan})
                      </span>
                    )}
                  </div>

                  <div className="flex items-center gap-1.5 text-[10px] text-slate-500 dark:text-slate-400">
                    <span className="font-semibold">FAN: {acFanSpeed.toUpperCase()}</span>
                    <span>•</span>
                    <span className="font-semibold">SWING: {acSwing ? 'ON' : 'OFF'}</span>
                    <span>•</span>
                    <span className="font-semibold">DIR: {acDir}</span>
                  </div>
                </div>
              </div>

              {/* LCD Bottom Row */}
              <div className="grid grid-cols-5 gap-1 pt-1.5 border-t border-dashed border-cyan-400/20 text-[9px] text-center font-bold relative z-10">
                <div className={`py-0.5 rounded transition ${acMode === 'cool' ? 'bg-cyan-500 text-white shadow-sm' : 'text-slate-400 bg-black/5 dark:bg-white/5'}`}>
                  COOL
                </div>
                <div className={`py-0.5 rounded transition ${acMode === 'heat' ? 'bg-orange-500 text-white shadow-sm' : 'text-slate-400 bg-black/5 dark:bg-white/5'}`}>
                  HEAT
                </div>
                <div className={`py-0.5 rounded transition ${acMode === 'dry' ? 'bg-sky-500 text-white shadow-sm' : 'text-slate-400 bg-black/5 dark:bg-white/5'}`}>
                  DRY
                </div>
                <div className={`py-0.5 rounded transition ${acMode === 'fan' ? 'bg-indigo-500 text-white shadow-sm' : 'text-slate-400 bg-black/5 dark:bg-white/5'}`}>
                  FAN
                </div>
                <div className={`py-0.5 rounded transition ${acMode === 'auto' ? 'bg-emerald-500 text-white shadow-sm' : 'text-slate-400 bg-black/5 dark:bg-white/5'}`}>
                  AUTO
                </div>
              </div>
            </div>
