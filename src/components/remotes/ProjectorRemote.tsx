...
export const ProjectorRemote: React.FC<ProjectorRemoteProps> = ({
  ...
  return (
    <div id="projector-remote-layout" className="w-full flex flex-col items-center space-y-3.5 animate-in fade-in duration-200">
      {/* PROJECTOR OPTICAL LCD DISPLAY */}
      <div
        id="projector-status-display"
        className={`w-full rounded-2xl p-3.5 border font-mono relative overflow-hidden transition-all shadow-inner ${
          isLight
            ? 'bg-gradient-to-b from-sky-50 via-white to-blue-50/60 border-sky-200/90 text-slate-800 shadow-sky-100/60'
            : 'bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 border-slate-800 text-white shadow-black'
        }`}
      >
        <div className="flex items-center justify-between text-[11px] pb-1.5 border-b border-slate-200 dark:border-slate-800">
          <div className="flex items-center gap-1.5 font-bold tracking-wider">
            <Maximize2 className="w-3.5 h-3.5 text-blue-500 animate-pulse" />
            <span className={isLight ? 'text-slate-900' : 'text-slate-200'}>
              PROJETOR CINEMA & OPTICS
            </span>
          </div>
        ...
        {power ? (
          <div className="pt-2.5 pb-0.5 space-y-2">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-[10px] text-slate-400 font-sans uppercase">Entrada de Sinal</div>
                <div className="text-sm font-black text-blue-600 dark:text-blue-400">
                  {source} <span className="text-[10px] text-slate-400 font-normal">({aspect})</span>
                </div>
              </div>

              <div className="flex items-center gap-2 text-right">
                <div>
                  <div className="text-[10px] text-slate-400 font-sans uppercase">Keystone</div>
                ...
                <div>
                  <div className="text-[10px] text-slate-400 font-sans uppercase">Foco</div>
                  <div className="text-xs font-bold text-sky-600 dark:text-sky-300">
                    {focus}/20
                  </div>
                </div>
              </div>
            ...
            {/* Status Flags */}
            <div className="flex items-center gap-2 text-[10px] pt-1 border-t border-slate-100 dark:border-slate-800/80">
              {isEco && (
                <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/50 px-1.5 py-0.5 rounded border border-emerald-300 dark:border-emerald-800">
                  <Leaf className="w-3 h-3" /> Eco Mode (80W)
                </span>
              )}
              {isFrozen && (
                <span className="flex items-center gap-1 text-amber-600 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/50 px-1.5 py-0.5 rounded border border-amber-300 dark:border-amber-800">
                  <Sparkles className="w-3 h-3" /> Freeze Ativo
                </span>
              )}
              {isBlank && (
                <span className="flex items-center gap-1 text-red-600 dark:text-red-300 bg-red-50 dark:bg-red-950/50 px-1.5 py-0.5 rounded border border-red-300 dark:border-red-800">
                  <EyeOff className="w-3 h-3" /> Tela Ocultada
                </span>
              )}
            </div>
          </div>
          ...
          <div className="py-3 text-center text-slate-500 text-xs font-mono">
            PROJETOR DESLIGADO (ESFRIANDO LÂMPADA)
          </div>
        )}
      </div>
